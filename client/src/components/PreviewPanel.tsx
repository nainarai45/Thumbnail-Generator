        ...
        <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
            ...
            {isLoading && (
                <div>
                    ...
                    <div className="text-center">
                        ...
                        <p className="mt-1 text-xs text-zinc-400">This may take 10-15 seconds.</p>
                    </div>
                </div>
            )}
            ...
            {!isLoading && thumbnail?.image_url && (
                <div className="group relative h-full w-full">
                    ...
                    <div className="absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                        <button onClick={onDownload} type = "button" className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font medium transition bg-white/30 ring-white/40 backdrop-blur hover:scale-105 active:scale-105 active:scale-95">
            ...
            {!isLoading && !thumbnail?.image_url && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-white/20 bg-black/25">
                    ...
                    <div className="px-4text-center">
                        <p className="font text-zinc-200">Generate your first thumbnail</p>
                        <p className="mt-1text-xs text-zinc-400">Fill out the form and click Generate</p>
                    </div>
                </div>
            )}
        </div>
